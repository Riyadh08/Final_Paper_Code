...
fragility metric (Omega) plus the existing MCKP DP solver (``dp_solver.py``).
...
6. solve_mckp()                -- exact knapsack allocation  (dp_solver.py)
...
from dp_solver import DPEntry, solve_mckp
...
def refinement_pass(
    ...
    3. For each such component re-measure its *marginal* Omega in the quantized
       ...
       Omega.
    4. Re-run the MCKP DP with the refined table.
