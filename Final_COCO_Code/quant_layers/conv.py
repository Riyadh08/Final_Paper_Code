...
class PTQSLBatchingQuantConv2d(PTQSLQuantConv2d):
    ...
    def _initialize_calib_parameters(self):
        ...
        # max(1, ...): with very large activations (e.g. COCO detection inputs)
        # the estimate can round down to 0, which would divide-by-zero below.
        self.parallel_eq_n = max(1, int((memory / 4) // numel))
