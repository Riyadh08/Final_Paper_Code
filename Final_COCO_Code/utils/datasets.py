...
class LoaderGenerator():
    ...
    def calib_loader(self, num=1024, batch_size=32, seed=3, in_memory=True):
        # Seed BOTH RNGs: numpy for index selection, torch for the random
        # train-transform augmentations (RandomResizedCrop / HorizontalFlip)
        # applied during the in-memory preload below. Without the torch seed the
        # 32 calibration crops differ run-to-run, shifting every Omega value.
        np.random.seed(seed)
