...
class AsymmetricallyBatchingQuantConv2d(PTQSLBatchingQuantConv2d):
    ...
    def calculate_percentile_weight_candidates(self, l=0.9, r=1.0):
        # Cap num_zp like linear.py/matmul.py do, otherwise at >=8-bit
        # (n_levels >= 128 > eq_n) num_scale underflows to 0 and the
        # candidate tensor comes out empty (IndexError on [-2]).
